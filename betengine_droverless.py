...
class BetEngine(WebsiteOpener):
        ...
        if not self.__browser_initialized:
            ...
            if self.__config.get("use_proxies", False):
                if account and account.proxy:
                    proxy = account.proxy
                    print(f"Using proxy from specified account {account.username}: {proxy}")
                else:
                    # Fallback to first available proxy
                    for acc in self.__accounts:
                        if acc.proxy:
                            proxy = acc.proxy
                            print(f"Using proxy from fallback account {acc.username}: {proxy}")
                            break
                if not proxy:
            ...
            if not hasattr(self, 'driver') or self.driver is None:
                ...
                if hasattr(self, 'setup_driver'):
                    success = self.setup_driver(self.__headless, proxy)
            ...
            # Get proxy again for reinitialization from specified account
            proxy = None
            if self.__config.get("use_proxies", False):
                if account and account.proxy:
                    proxy = account.proxy
    ...
    def __setup_accounts(self):
        ...
        if self.__accounts and not self.__skip_initial_login:
            ...
            try:
                ...
                self._initialize_browser_if_needed(None)
                ...
                if self.__browser_initialized:
                    print("Closing browser after account setup...")
                    self.cleanup()
        elif self.__skip_initial_login:
            print("Skipping initial login as requested")
        else:
            print("No accounts configured for initial login")
    ...
    def __fetch_account_balance(self, account):
        ...
        try:
            if not account.cookie_jar:
                print(f"No cookies available for account {account.username}, cannot fetch balance")
                return 0
            ...
            if response.status_code == 200:
                ...
                if balance_data.get("bizCode") == 10000 and "data" in balance_data:
                    ...
                    print(f"Account {account.username} balance: {real_balance:.2f} (avlBal: {avl_bal}, bizCode: {biz_code})")
                    
                    return real_balance
                else:
                    print(f"Invalid balance response for account {account.username}: {balance_data}")
                    return 0
            else:
                print(f"Failed to fetch balance for account {account.username}: HTTP {response.status_code}")
                return 0
    ...
    def __search_event(self, home_team, away_team, pinnacle_start_time=None):
                ...
                if response.status_code == 200:
                    ...
                    if "data" in search_results and search_results["data"] and "events" in search_results["data"]:
                        ...
                        for event in events:
                            ...
                            if pinnacle_start_time and "startTime" in event:
                                ...
                                try:
                                    ...
                                    if time_diff_hours <= 1.0833:  # Within 1 hour and 5 minutes
                                        ...
                                        print(f"Time difference: {time_diff_hours:.2f} hours, not the right game")
                            ...
                            print(f"Potential match: {event_name} (Score: {match_score})")
                    else:
                        print(f"No data found in search response for term: {search_term}")
                else:
                    print(f"Search request failed with status: {response.status_code}")
                    # print(f"Response content: {response.text[:500]}...")
            
            except Exception as e:
                print(f"Error searching for event with term '{search_term}': {e}")
                import traceback
                traceback.print_exc()
                continue
        ...
        print("No matching event found on MSport")
        return None
    ...
    def __get_event_details(self, event_id):
        ...
        try:
            ...
            print(f"Getting event details with URL: {details_url} and params: {params}")
            response = requests.get(details_url, params=params, headers=headers)
            
            print(f"Event details response status: {response.status_code}")
            if response.status_code == 200:
                try:
                    event_details = response.json()
                    # print(f"Event details response: {event_details}")
                except ValueError as e:
                    print(f"Failed to parse JSON response for event details: {e}")
                    print(f"Response content: {response.text[:500]}...")
                    return None
                ...
                if "data" in event_details:
                    ...
                    print(f"No 'data' field in event details response")
                    return None
            else:
                print(f"Failed to get event details: HTTP {response.status_code}")
                print(f"Response content: {response.text[:500]}...")
                return None
    ...
    def __generate_msport_bet_url(self, event_details):
        ...
        try:
            ...
            print(f"Generated MSport bet URL: {bet_url}")
            return bet_url
    ...
    async def __place_bet_with_selenium(self, account, bet_url, market_type, outcome, odds, stake, points=None):
        ...
        try:
            ...
            try:
                print(f"Found market element for: {market_type} - {outcome} - {points}")
                ...
                try:
                    ...
                    if odds_match:
                        ...
                        if odds_diff > 0.1:  # Allow 0.1 difference
                            print(f"⚠️ Odds mismatch! Expected: {odds}, Actual: {actual_odds}")
                            print("Bet cancelled due to odds change")
                            return False
                        else:
                            print(f"✅ Odds verified: {actual_odds} (expected: {odds})")
                except Exception as e:
            ...
            try:
                ...
                try:
                    ...
                    print(f"Could not find bet button with specific class: {e}")
                ...
                try:
                    ...
                    print(f"Error checking for success confirmation: {e}")
                    return False
                    
            except Exception as e:
                print(f"Error clicking place bet button: {e}")
                return False
    ...
    async def __get_market_selector(self, market_type, outcome, points=None, is_first_half=False):
        ...
        try:
            ...
            print(f"Error finding market rows: {e}")
            return None
    ...
    async def __find_1x2_outcome(self, market_row, outcome):
        ...
        """
        if not market_row:
            print("No 1X2 market row provided")
            return None
        ...
        try:
            ...
            if outcome not in outcome_index:
                print(f"Invalid 1X2 outcome: {outcome}")
                return None
            ...
            try:
                ...
                print(f"Could not find odds in 1X2 outcome: {e}")
                return None
    ...
    async def __find_total_outcome(self, target_points, outcome):
        ...
        try:
                    ...
                    if match:
                        ...
                        if abs(row_points - float(target_points)) < 0.01:
                            ...
                            try:
                                ...
                                print(f"Could not find odds in total outcome: {e}")
                                continue
            ...
            print(f"Could not find total market for {target_points} {outcome}")
            return None
    ...
    async def __find_handicap_outcome(self, target_points, outcome):
        ...
        try:
                        ...
                        if match:
                            ...
                            if abs(row_points - float(target_points)) < 0.01:
                                ...
                                try:
                                    ...
                                    print(f"Could not find odds in handicap outcome: {e}")
                                    continue
            ...
            print(f"Could not find handicap market for {target_points} {outcome}")
            return None
    ...
    def __find_market_bet_code_with_points(self, event_details, line_type, points, outcome, is_first_half=False, sport_id=1, home_team=None, away_team=None):
        ...
        if "markets" not in event_details:
            print("No markets found in event details")
                ...
                if market_desc == moneyline_market.lower():
                    ...
                    for market_outcome in market.get("outcomes", []):
                        if market_outcome.get("id") == target_outcome_id:
                            print(f"Found moneyline market: {market['description']} outcome {target_outcome_id} with odds {market_outcome['odds']}")
        ...
        # Handle TOTAL bets (Over/Under in MSport)
            ...
            print(f"Original points: {original_points}, Rounded to nearest 0.5: {rounded_points}")
            ...
            print(f"Searching for total points in order: {alternate_points}")
            ...
            if best_match:
                if best_match["points"] != original_points:
                    print(f"Exact total {original_points} not found, using closest: {best_match['points']}")
                print(f"Found total market: {best_match['description']} with odds {best_match['odds']}")
                return best_match["id"], best_match["odds"], best_match["points"]
            
            print(f"No matching total market found for {points} {outcome} or alternate lines")
            return None, None, None
        ...
        # Handle SPREAD bets (Asian Handicap in MSport)
            ...
            rounded_points = round(original_points * 2) / 2
            print(f"Original handicap: {original_points}, Rounded to nearest 0.5: {rounded_points}")
            ...
            print(f"Searching for handicap points in order: {alternate_points}")
            ...
            if best_match:
                if best_match["points"] != original_points:
                    print(f"Exact handicap {original_points} not found, using closest: {best_match['points']}")
                print(f"Found handicap market: {best_match['description']} with odds {best_match['odds']}")
                return best_match["id"], best_match["odds"], best_match["points"]
            
            print(f"No matching handicap market found for {points} {outcome} or alternate lines")
            return None, None, None
        ...
        for account in self.__accounts:
            if account.cookie_jar:
                try:
                    ...
                    print(f"Balance updated for {account.username}: {old_balance:.2f} -> {new_balance:.2f} (change: {new_balance - old_balance:+.2f})")
                except Exception as e:
                    print(f"Failed to refresh balance for {account.username}: {e}")
                    balance_updates[account.username] = {
                        "error": str(e)
                    }
            else:
                print(f"No valid cookies for {account.username}, skipping balance refresh")
