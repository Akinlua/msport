...
url = (
    "https://apigw.bet9ja.com/"
    "sportsbook/placebet/PlacebetV2"
    "?source=desktop&v_cache_version=1.274.3.186"
)

# Prepare your BETSLIP JSON as a string:
betslip_json = json.dumps({
    "BETS": [
        {
            "BSTYPE": 3, "TAB": 3, "NUMLINES": 1,
            "COMB": 1, "TYPE": 1, "STAKE": 10,
            "POTWINMIN": 32.5, "POTWINMAX": 32.5,
            "BONUSMIN": "0", "BONUSMAX": "0",
            "ODDMIN": "3.25", "ODDMAX": "3.25",
...
# Build a 'files' dict where each key is a form-field name,
# and the value is a tuple (filename, content, content_type).
# If you don’t want to send a file on disk, just put filename=None.
files = {
    "BETSLIP": (None, betslip_json, "application/json"),
    "BONUS":    (None, "0"),
    "ACCEPT_ODDS_CHANGES": (None, "1"),
    "IS_PASSBET": (None, "0"),
    "IS_FIREBETS": (None, "0"),
    "IS_CUT1": (None, "0"),
}

# Copy over your cookies or any other headers, but
# **do not** manually set Content-Type here—requests will set it.
headers = {
...
try:
    resp = requests.post(
        ...
        verify=True         # or False if you suspect SSL issues
    )
    resp.raise_for_status()
    print("✅ HTTP", resp.status_code)
    print(resp.text)      # or resp.json() if it returns JSON
except requests.exceptions.Timeout:
    print("⚠️  Timed out (either connecting or waiting for data)")
